...
public class GamePlay_Model extends JPanel implements ActionListener, MouseListener {
	...
	// GAME_UNITS its like doing: (600 * 600) / 25 = 14.400
	...
	public void draw(Graphics g) {
		...
		if (running) {
			...
			for (int i = 0; i < bodyParts; i++) {
				...
				/*
				 * // Ajustar las coordenadas para alinear los cuadros con los bordes
				int adjustedX = x[i] - (x[i] % UNIT_SIZE);
				int adjustedY = y[i] - (y[i] % UNIT_SIZE);

				g.fillRect(adjustedX, adjustedY, UNIT_SIZE, UNIT_SIZE);
				 */
	...
	private void isGameOver() {
		...
		// Save the player's nickname & score and send it to GameScore_Model
		GameScore_Model gameScore = new GameScore_Model(playerName, applesEaten);
		gameScore.saveScoreToDatabase();
	}
